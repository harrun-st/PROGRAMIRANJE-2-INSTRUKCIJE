...
string GenerisiClanskiBroj(string imePrezime, int redniBroj)
{
	string Sifra;
	//GGGG
	time_t trenutna = time(0);
	tm utcvrijeme;
	localtime_s(&utcvrijeme, &trenutna);

	int godina = utcvrijeme.tm_year + 1900;
	...
	char prezime = imePrezime[imePrezime.find(' ') + 1];
...
{
	regex sifra("[0-9]{4}[/][A-Z]{2}[-][0-9]{3}");

	return regex_match(clanskiBroj, sifra);
}
...
public:
	...
	Kolekcija<T1, T2, max> DodajNaPoziciju(int lokacija, T1 el1, T2 el2)
	{
		Kolekcija<T1, T2, max> kolekcija1;
	...
	Kolekcija<T1, T2, max>* UkloniRaspon(int lokacijaOd, int elementi)
	{
		Kolekcija<T1, T2, max> kolekcija1;
		Kolekcija<T1, T2, max>* kolekcija3 = new Kolekcija<T1, T2, max>();

		for (int i = 0; i < _trenutno; i++)
		{
			if (i > lokacijaOd && i < lokacijaOd + elementi)
			{
				kolekcija3->Dodaj(_prvi[i], _drugi[i]);
			}
			else
			{
				kolekcija1.Dodaj(_prvi[i], _drugi[i]);
...
public:
	...
	{
		ostringstream ss;
		ss << setfill('0') << setw(2) << _datumPosudbe.GetDan() << ".";
		ss << setfill('0') << setw(2) << _datumPosudbe.GetMjesec() << ".";
		ss << setfill('0') << setw(2) << _datumPosudbe.GetGodina() << " ";


		ss << _naslov << " " << ZanrNazivi[_zanr]<<" "<<_brojDana<<" dana";
...
public:
	...
	bool DodajPosudbu(Posudba posudba)
	{
		...
		if (posudba.GetBrojDana() < 1 || posudba.GetBrojDana() > 30)
			return false;
...
public:
	...
	bool DaLiClanPostoji(const ClanBiblioteke clan)
	{
		for (auto& c : _clanovi)
		{
			if (c.GetClanskiBroj(), clan.GetClanskiBroj() == 0)
			{
				return true;
			}
		}
	...
	{
		if (DaLiClanPostoji(clan) == true)
		{
			throw exception("Ne moze jer vec ima clanski broj isti");
		}
	...
	bool EvidentirajPosudbu(const char* clanskibroj, Posudba naslov)
	{
		for (auto& c : _clanovi)
		{
			if (strcmp(c.GetClanskiBroj(), clanskibroj) == 0)
			{
				_clanovi.push_back(naslov);
			}
		}
	}
