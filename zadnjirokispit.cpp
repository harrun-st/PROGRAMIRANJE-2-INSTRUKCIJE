...
ostream& operator <<(ostream& COUT, const VrstaDostave& obj)
{
	COUT << VrstaDostaveNazivi[obj];
...
string GenerisiOznaku(const char* imePrezime, int redniBroj, VrstaDostave vrstaDostave)
{
	...
	if (redniBroj < 1 || redniBroj>9999 || vrstaDostave>MEDJUNARODNA || strchr(imePrezime,' ')==nullptr)
	{
	...
	ostringstream ss;
	//VD
	Sifra += VrstaDostaveOznake[vrstaDostave];
	Sifra += "-";
	...
	size_t razmak = prezime.find_last_of(' ');
	
	Sifra += imePrezime[0];
	Sifra += imePrezime[razmak + 1];
...
public:
	...
	*/
	Kolekcija<int, string, 5> DodajNaPoziciju(int pozicija, T1 el1, T2 el2)
	{
		Kolekcija<int, string, 5> kolekicjua1;

		for (int i = 0; i < _trenutno; i++)
		{
			if (i == pozicija)
			{
				kolekicjua1.Dodaj(el1, el2);
			}
			kolekicjua1.Dodaj(*_elementi1[i], *_elementi2[i]);
		}
		return kolekicjua1;
	}
	/*
	UkloniSaPozicije uklanja par sa proslijedjene pozicije/indeksa,
	preostale elemente pomjera ulijevo i vraca uklonjeni par u obliku pair<T1, T2>.
	Za poziciju izvan opsega baca se izuzetak.
	*/
	
	pair<int, string> UkloniSaPozicije(int pozicija)
	{
		if (pozicija < 1 || pozicija >= _trenutno)
		{
		...
		for (int i = 0; i < _trenutno; i++)
		{
			if (i != pozicija)
			{
				preostali.Dodaj(*_elementi1[i], *_elementi2[i]);
			}
		}
		*this = preostali;
...
public:
	...
	int GetSumaDana() const {

		return *_dan + *_mjesec * 30 + *_godina * 365;

	}
	int GetSumaMinuta() const {

		return *_minute + *_sati * 60 + GetSumaDana() * 24 * 60;

	}

	bool operator > (const DatumVrijeme& obj) const {

		return GetSumaMinuta() > obj.GetSumaMinuta();
	...
	string ToString()const
	{
		...
		ss << setfill('0') << setw(2) << *_dan << "." << setw(2) << *_mjesec << "." << *_godina << " " << setw(2) << *_sati << ":" << setw(2) << *_minute;
...
public:
	...
	Posiljka(const char* sadrzaj,const char* imeprezime,const char* email,VrstaDostave vrstadostave,int rednibroj,DatumVrijeme datum)
	{
	...
	string ToString()const
	{
		...
		ss << _oznaka << " | " << _imePrezimePrimaoca << " | " << _vrstaDostave << " | " << _sadrzaj << " | " << GetTrenutniStatus();
	...
	DatumVrijeme GetTrenutnoDatumVrijeme()const
	{
		return _statusi.GetElement2(_statusi.GetTrenutno() - 1);
	}
	/*
	...
	*/
	bool DodajStatus(StatusPosiljke status, DatumVrijeme datum)
	{
		DatumVrijeme posljednjevrijeme = GetTrenutnoDatumVrijeme();

		StatusPosiljke posljednjistatus = GetTrenutniStatus();


		if (posljednjevrijeme > datum)
		{
			return false;
		}

		if (posljednjistatus == ISPORUCENA || posljednjistatus == OTKAZANA)
		{
			return false;
		}
		...
		if (status == OTKAZANA)
		{
			_statusi.Dodaj(status, datum);
			return true;
		}

		if (posljednjistatus == KREIRANA && status == PREUZETA)
		{
			_statusi.Dodaj(status, datum);
			return true;
		}
		else if (posljednjistatus == PREUZETA && status ==U_TRANZITU)
		{
			_statusi.Dodaj(status, datum);
			return true;
		}
		else if (posljednjistatus == U_TRANZITU && status == ISPORUCENA)
		{
			_statusi.Dodaj(status, datum);
			return true;
		}
...
public:
	...
	void DodajPosiljku(Posiljka posiljka)
	{
		for (auto& p : _posiljke)
		{
			...
			{
				throw exception("isti rbr i oznakaaaa");
			}
		}
	...
	void PosaljiMail(Posiljka posiljka)
	{
		m.lock();

		cout << crt << "To: " << posiljka.GetEmailPrimaoca() << endl;
		cout << "From: info@brzaposta.ba" << endl;
		cout << "Subject: Promjena statusa posiljke" << endl;
		cout << "Postovani " << posiljka.GetImePrezimePrimaoca() << "," << endl;
		cout << "Posiljka " << posiljka.GetOznaka() << " je promijenila status u " << posiljka.GetTrenutniStatus() << endl;
		cout << "Vrijeme evidentiranja: " << posiljka.GetTrenutnoDatumVrijeme() << endl;
		cout << "Hvala sto koristite nase usluge." << endl;
		cout << _naziv << " tim" << crt;
	...
	*/
	vector<Posiljka*> PosiljkePoStatusu(StatusPosiljke status)
	{
		...
		for (auto& p : _posiljke)
		{
			...
			{
				preuzetePosiljke.push_back(&p);
			}
		}
		return preuzetePosiljke;
	}
	...
	//	for (int i = 0; i < 5; i++)
	//	{
	//		StatusPosiljke svakistatus = (StatusPosiljke)i;
...
int main() {
	...
	*/
	...
	if (GenerisiOznaku("Ana Marija Kovac", 156, MEDJUNARODNA) == "BH-MD-0156-AK")
		cout << "Oznaka OK" << crt;
	if (GenerisiOznaku("Amina", 42, BRZA) == "BH-XX-0000-XX" && GenerisiOznaku("Amina Buric", 0, BRZA) == "BH-XX-0000-XX" && GenerisiOznaku("Amina Buric", 10000, BRZA) == "BH-XX-0000-XX" && GenerisiOznaku("Amina Buric", 42, (VrstaDostave)99) == "BH-XX-0000-XX")
	...
	*/
	...
	if (!ValidirajOznaku("BH-EX-0042-AB") && !ValidirajOznaku("BH-BR-042-AB") && !ValidirajOznaku("BH-BR-0042-Ab") && !ValidirajOznaku("BH-BR-0000-AB") && !ValidirajOznaku("bh-BR-0042-AB"))
	...
	*/
	Posiljka dokumenti("Dokumenti", "Amina Buric", "amina@fit.ba",
		BRZA, 42, kreirana);
	Posiljka knjige("Knjige", "Goran Skondric", "goran@fit.ba",
		STANDARDNA, 7, kreirana);
	Posiljka oprema("Oprema", "Ana Marija Kovac", "ana@fit.ba",
		MEDJUNARODNA, 156, kreirana);
	/*
	ToString vraca podatke u formatu:
	oznaka | ime i prezime | vrsta dostave | sadrzaj | trenutni status
	BH-BR-0042-AB | Amina Buric | BRZA | Dokumenti | KREIRANA
