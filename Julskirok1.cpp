...
/*
    ...
    */
string GenerisiOznaku(string imePrezime, int rednibroj, VrstaDostave dostava)
{
    if (rednibroj < 1 || rednibroj>9999)  return "BH-XX-0000-XX";
    if (dostava<STANDARDNA || dostava>MEDJUNARODNA) return "BH-XX-0000-XX";
    string sifra;

    sifra += "BH-";

    //VD
    if (dostava == STANDARDNA) sifra += "ST";
    else if (dostava == BRZA) sifra += "BR";
    else if (dostava == MEDJUNARODNA)sifra += "MD";
    sifra += "-";
    ...
    size_t zadnjirazmak = imePrezime.find_last_of(' ');

    char ime = imePrezime[0];
    char prezime = imePrezime[zadnjirazmak + 1];
