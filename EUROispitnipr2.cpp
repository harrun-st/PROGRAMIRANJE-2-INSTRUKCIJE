...
char NadjiPrvoSlovoPrezimena(const char* imeprezime)
{
...
{
	char PrvoSlovoIme = toupper(ImePrezime[0]);

	char PrvoSlovoPrezime = toupper(NadjiPrvoSlovoPrezimena(ImePrezime));

	int ObrnutiBroj = OkreniBroj(id);

	char temp[10];

	if (id < 10) { // treba vratiti D000M3

		sprintf_s(temp, "%c000%c%d", PrvoSlovoIme, PrvoSlovoPrezime, ObrnutiBroj); // %c - char , %s - string , %d - decimal

	}
	else if (id < 100) { // treba vratiti J00A41

		sprintf_s(temp, "%c00%c%d", PrvoSlovoIme, PrvoSlovoPrezime, ObrnutiBroj); // %c - char , %s - string , %d - decimal

	}
	else if (id < 1000) { // treba vratiti G0S651


		sprintf_s(temp, "%c0%c%d", PrvoSlovoIme, PrvoSlovoPrezime, ObrnutiBroj); // %c - char , %s - string , %d - decimal

	}
	else { // treba vratiti EJ8971

		sprintf_s(temp, "%c%c%d", PrvoSlovoIme, PrvoSlovoPrezime, ObrnutiBroj); // %c - char , %s - string , %d - decimal
...
public:
	...
	Kolekcija RemoveRange(int lokacijaOd, int lokacijaDo)
	{
